...
# "ถามตำแหน่งที่บันทึกทุกครั้ง" ไว้ จะเด้งหน้าต่างเลือกที่เก็บทุกคลิป
# = คิวหยุดรอคนกด ไม่ได้ทำงานอัตโนมัติจริง
# ใช้นโยบายระดับผู้ใช้ (defaults) ไม่ไปแก้ไฟล์โปรไฟล์เบราว์เซอร์ — ถอนคืนง่าย ไม่เสี่ยงทำโปรไฟล์พัง
for b in com.google.Chrome com.microsoft.Edge; do
  defaults write "$b" PromptForDownloadLocation -bool false 2>/dev/null || true
done
ok "ปิดหน้าต่างถามที่บันทึกไฟล์ของ Chrome/Edge แล้ว (ปิด-เปิดเบราว์เซอร์ 1 ครั้งถึงมีผล)"
