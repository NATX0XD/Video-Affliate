...
แอปนี้ "ไม่ได้เซ็นดิจิทัล" (unsigned) — เป็นเรื่องปกติสำหรับเครื่องมือภายใน

วิธีเปิดครั้งแรก (กัน Gatekeeper เตือน "แอปจากผู้พัฒนาที่ไม่รู้จัก"):
  1) คลิกขวา (หรือ Control+คลิก) ที่ "Install VDO Gen Auto Pilot.app"
  2) เลือก  Open  แล้วกด  Open  ซ้ำในกล่องเตือน
  (ครั้งต่อ ๆ ไปดับเบิลคลิกเปิดได้เลย)

  ★ ถ้าเป็น macOS 15 (Sequoia) ขึ้นไป แล้วคลิกขวา > Open ไม่มีปุ่มให้เปิด:
     ไปที่ เมนู Apple > System Settings > Privacy & Security เลื่อนลงล่างสุด
     จะเห็นข้อความว่า "Install VDO Gen Auto Pilot" ถูกบล็อก → กดปุ่ม  Open Anyway
     แล้วยืนยันด้วยรหัสเครื่อง/Touch ID (ทำครั้งเดียว)

หมายเหตุ: ครั้งแรกอาจมีป๊อปอัป "…wants to control Terminal" ให้กด  OK / Allow
  (ตัวติดตั้งใช้ Terminal เพื่อแสดงหน้าจอความคืบหน้า — ต้องอนุญาต)

ตัวติดตั้งจะทำอะไร:
  • ดึงโค้ดล่าสุดจาก GitHub มาลงที่  ~/Applications/VDO-Gen-AutoPilot
  • ติดตั้งเครื่องมือ (adb / scrcpy / ffmpeg + Python venv) ลงในเครื่องผู้ใช้เอง

ก่อนติดตั้ง โปรดตรวจ:
  • ต้องต่ออินเทอร์เน็ต (ตัวติดตั้งจะโหลดโค้ด + เครื่องมือจากเน็ต)
  • ไม่ต้องใช้รหัสเครื่อง (admin) — ลงในโฟลเดอร์ผู้ใช้ ไม่แตะระบบ
EOF
ok "อ่านก่อน-Mac.txt"
