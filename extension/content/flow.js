...
if (window._flowAutomatorLoaded) {
  ...
  async function runGenerate({ prompt, imageDataUrl, charImageDataUrl, bgImageDataUrl, moodImageDataUrl, productId, _log, dry }) {
    ...
    // รูปฉากหลังที่ผู้ใช้อัป → ภาพอ้างอิงฉาก (prompt จาก background ระบุว่ารูปสุดท้าย = ฉากหลัง)
    if (bgImageDataUrl) {
  ...
  const MODE_ALT = {
    ...
    // Flow เปลี่ยนชื่อโหมดย่อยคู่กับ "เฟรม" จาก "ส่วนผสม" เป็น "องค์ประกอบ"
    // อ่านสถานะโหมดย่อยด้วยชื่อเก่าอย่างเดียวจะได้ค่าว่าง แล้วตรวจไม่ออกว่าอยู่โหมดไหน
    "เฟรม": ["เฟรม", "frames"],
    "ส่วนผสม": ["ส่วนผสม", "องค์ประกอบ", "ingredients", "elements"],
    "องค์ประกอบ": ["องค์ประกอบ", "ส่วนผสม", "ingredients", "elements"],
  ...
  // ── รวมรูปเป็นภาพเดียว เรียงซ้าย→ขวา (หน้า | สินค้า | ฉากหลังที่ผู้ใช้อัป) → แนบ reference ครั้งเดียว ──
  // แนบรูปเดียวเชื่อถือได้กว่าแนบหลายรอบ (เลี่ยงเมนู ⋮ portal ที่ไล่จับยากเมื่อมีหลาย tile)
  async function combineRefs(...urls) {
    ...
    cv.width = ws.reduce((a, b) => a + b, 0) + gap * (imgs.length - 1); cv.height = H;
    const ctx = cv.getContext("2d");
    ctx.fillStyle = "#ffffff"; ctx.fillRect(0, 0, cv.width, cv.height);
    let x = 0;
    imgs.forEach((im, i) => { ctx.drawImage(im, x, 0, ws[i], H); x += ws[i] + gap; });
  ...
  const extraDirective = (extra) => (extra && extra.trim()
    ? ` · ★★ คำสั่งที่ผู้ใช้เขียนเอง (ยึดตามนี้ก่อนเสมอ): ${extra.trim()}`
    : "");
  ...
  // หัวข้อพรอมป์เขียนเองที่ "มีผลกับภาพนิ่ง" → ต่อท้ายพรอมป์เฟรม (ฉากแยกไปอยู่ตัวแปร bg แล้ว)
  const FRAME_PROMPT_KEYS = [
    ["light", "แสง"], ["camera", "กล้อง/มุมภาพ"], ["char", "ตัวละคร"],
  ...
  async function resolveComposeInputs(opts) {
    ...
    try {
      ...
      if (!extra) {
        const pr = g.prompts || {};
        extra = FRAME_PROMPT_KEYS.filter(([k]) => (pr[k] || "").trim())
  ...
  const defaultMotionPrompt = (name) =>
    ...
    // อาการที่เจอจริง: สินค้าลอยหลุดมือ แขนงอกเพิ่ม นิ้วเกิน และพูดประโยคเดิมวนซ้ำจนครบความยาวคลิป
    `ฟิสิกส์ต้องถูก: สินค้าอยู่ในมือตลอด มีจุดสัมผัสชัดเจน ห้ามลอยเอง ห้ามหลุดมือ ห้ามทะลุผ่านมือ · ` +
    `ร่างกายมี 2 แขน 2 มือเท่านั้น มือละ 5 นิ้ว ห้ามงอกเพิ่ม ห้ามข้อต่อบิดผิดทาง · ` +
    `ขนาด/รูปทรง/สี/ฉลากสินค้าคงเดิมทุกเฟรม ห้ามมอร์ฟ ห้ามยืดหด · ขยับช้าและน้อย · ` +
