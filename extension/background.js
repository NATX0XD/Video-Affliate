...
async function geminiFlowPrompt(product, cfg, gen, hasCharImage, charImg, i2v = false) {
  ...
  const dialogueSpec = mute
    ? '""'
    : `"<บทพูด${langName !== 'ไทย' ? `${langName} (${langLine})` : 'ไทย'} พูดจริงได้แค่ ~${words} คำ และต้องพูดจบภายใน ${fmtT(speakEnd)} วิ (เหลือท้าย ~${fmtT(tailSec)} วิ เป็นภาพปิดเงียบ ห้ามมีคำพูด) — ใช้สูตรขาย 3 จังหวะ H-V-C: [HOOK เลือก 1 ฮุกที่เข้ากับสินค้าตัวนี้สุด จาก: ปัญหาแหย่ใจ('เบื่อไหม...')/คำสั่งหยุด('หยุดเลื่อน!')/ผลลัพธ์เกินคาด/คำถามกระตุก('รู้ไหมว่า...')/ความรู้สึกตอนได้ใช้ — ต้องสะดุดใน 1 วิแรก] + [VALUE จุดขายเด็ดสุด 'จุดเดียว' พูดเป็นประโยชน์ที่เห็นภาพได้ ไม่ใช่สเปกเทคนิค (พูดผลลัพธ์ที่ผู้ใช้ได้ ไม่ใช่ตัวเลขสเปก)] + [CLOSE ความคุ้ม/ความน่าได้แบบไม่อ้างตัวเลข + คำเร่งด่วน(ก่อนหมด/เฉพาะวันนี้) + สั่งกดตะกร้าตรงๆ] — ${NO_PRICE_RULE} · ${NO_REPEAT_RULE} · ห้ามเกริ่นยาว ห้ามอ่านสรรพคุณเป็นลิสต์ ห้ามเคลมเกินจริง(ขาวใน X วัน/รักษา/100%) บทพูดต้องจบไม่เกิน ${fmtT(speakEnd)} วิ ห้ามค้างกลางคำตอนคลิปจบ>"`;
