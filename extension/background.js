...
async function geminiFlowPrompt(product, cfg, gen, hasCharImage, charImg, i2v = false) {
  ...
  const STYLE_SPEC = {
    hardsell: {
      ...
      dialogue: 'ประโยคสั้นกระแทก เสียงดังมั่นใจ ย้ำความคุ้ม+ความเร่งด่วน สั่งกดตะกร้าตรงๆ แบบไม่อ้อม (ห้ามพูดราคา)',
    ...
    shock: {
      name: 'ตกใจราคา',
      open: 'เฟรมแรกตัวละครทำสีหน้าตกใจ/อึ้งเกินจริง (double-take) มองกล้องค้าง แบบไม่อยากเชื่อว่าของแบบนี้มีจริง',
      ...
      dialogue: 'เปิดด้วยอุทานตกใจในตัวสินค้า แล้วเฉลยว่าทำไมคุ้มขนาดนี้ ปิดด้วยรีบกดก่อนหมด (ห้ามพูดราคา)',
      focus: 'สีหน้าตกใจ = ฮุก แล้วดึงสายตามาที่ตัวสินค้า',
    },
    demo: {
      name: 'สาธิตของ',
  ...
  // ── กันบทพูดซ้ำข้ามคลิป — เก็บบทที่เพิ่งใช้ไว้ 12 อัน แล้วห้าม Gemini เขียนแนวเดิมซ้ำ ──
  // ไม่งั้นสินค้าคนละตัวก็ได้ฮุกเดียวกันหมด ("หยุดเลื่อน!" ทุกคลิป) คนดูจับได้ว่าเป็นบอท
  let recentBlock = '';
  try {
    const rs = (await chrome.storage.local.get('flow_recent_scripts')).flow_recent_scripts || [];
    const lines = rs.slice(-12).map((s) => String(s || '').trim()).filter(Boolean);
    if (lines.length)
      recentBlock = `\n★ บทพูดที่เพิ่งใช้ไปในคลิปก่อนหน้า — ห้ามเขียนซ้ำ ห้ามใช้ฮุกเดิม ห้ามขึ้นต้นเหมือนกัน ต้องคิดมุมใหม่:\n${lines.map((l) => `- ${l.slice(0, 90)}`).join('\n')}\n`;
  } catch {}
  // ── กฎบทพูดที่ใช้ร่วมกันทุก schema ──────────────────────────────────────
  // ราคาเปลี่ยนได้ตลอด (โปรหมด/ปรับราคา) แต่คลิปอยู่ยาว — พูดราคาไปแล้วคลิปเก่ากลายเป็นข้อมูลผิดทันที
  // ราคาโชว์ในแคปชั่น/หน้าร้านอยู่แล้ว ไม่ต้องพูดซ้ำในคลิป
  const NO_PRICE_RULE = 'ห้ามพูดราคาหรือตัวเลขเงินเด็ดขาด (ห้ามคำว่า บาท/ลด/เหลือ/แค่...บาท/เปอร์เซ็นต์ส่วนลด) — ขายด้วยประโยชน์และความรู้สึก ไม่ใช่ตัวเลข';
  // Veo หลุดฟิสิกส์บ่อยตอนขยับแรง — สินค้าลอยหลุดมือ แขนงอกเพิ่ม นิ้วเกิน สินค้าเปลี่ยนทรงกลางคลิป
  const PHYSICS_RULE = 'ฟิสิกส์ต้องถูก: สินค้าต้องอยู่ในมือตลอด มีจุดสัมผัสชัดเจน ห้ามลอยเอง ห้ามหลุดมือ ห้ามทะลุผ่านมือ · ร่างกายมี 2 แขน 2 มือเท่านั้น มือละ 5 นิ้ว ห้ามงอกเพิ่ม ห้ามซ้อนทับ ห้ามบิดผิดข้อ · ขนาด/รูปทรง/สี/ฉลากของสินค้าคงเดิมทุกเฟรม ห้ามมอร์ฟ ห้ามยืดหด · ขยับช้าและน้อย (ยิ่งขยับแรง มือ/หน้ายิ่งเพี้ยน)';
  ...
  // Veo ชอบวนพูดประโยคเดิมซ้ำเมื่อบทสั้นกว่าความยาวคลิป → บทต้องเต็มหน้าต่างพูดพอดี
  const NO_REPEAT_RULE = `พูดครั้งเดียวจบ ห้ามพูดประโยคเดิมซ้ำ ห้ามวนลูปคำพูด ห้ามขึ้นต้นซ้ำคำเดิม ทุกประโยคในบทต้องต่างกัน · `
    + `บทต้องยาวพอเต็มช่วงพูด (~${words} คำ ไม่ต่ำกว่านี้มาก) — บทสั้นกว่าความยาวคลิปคือสาเหตุที่ Veo วนพูดซ้ำเอง`;
  ...
  const dialogueSpec = mute
    ? '""'
    : `"<บทพูด${langName !== 'ไทย' ? `${langName} (${langLine})` : 'ไทย'} พูดจริงได้แค่ ~${words} คำ และต้องพูดจบภายใน ${fmtT(speakEnd)} วิ (เหลือท้าย ~${fmtT(tailSec)} วิ เป็นภาพปิดเงียบ ห้ามมีคำพูด) — ใช้สูตรขาย 3 จังหวะ H-V-C: [HOOK แต่งประโยคเปิดขึ้นใหม่ให้เฉพาะกับสินค้าตัวนี้ — เลือกชนิดฮุกที่เข้ากับสินค้าที่สุด: จี้ปัญหาที่คนกลุ่มนี้เจอจริง / เผยผลลัพธ์ที่เกินคาด / ตั้งคำถามที่คนอยากรู้คำตอบ / เล่าความรู้สึกวินาทีแรกที่ได้ใช้ / เทียบกับวิธีเดิมที่คนทำกันอยู่ · ห้ามเปิดด้วยคำสั่งให้หยุดดู (หยุดก่อน/หยุดเลื่อน/อย่าเพิ่งเลื่อน) และห้ามใช้ประโยคเปิดเดิมซ้ำกับคลิปก่อน ๆ · ต้องสะดุดใน 1 วิแรก] + [VALUE ต้องบอกให้คนดูรู้ว่า "นี่คือสินค้าอะไร ใช้ทำอะไร" ด้วยคำสั้น ๆ ก่อน (เรียกชื่อประเภทสินค้าตรง ๆ เช่น เครื่องโกนหนวด/รองเท้าหนัง/พัดลมพกพา — คนเลื่อนผ่านต้องเข้าใจใน 2 วิว่ากำลังดูอะไรอยู่) แล้วต่อด้วยจุดขายเด็ดสุด 'จุดเดียว' เป็นประโยชน์ที่เห็นภาพได้ ไม่ใช่สเปกเทคนิค (พูดผลลัพธ์ที่ผู้ใช้ได้ ไม่ใช่ตัวเลขสเปก) · รวมกันต้องกระชับ ไม่ใช่ท่องสรรพคุณ] + [CLOSE ความคุ้ม/ความน่าได้แบบไม่อ้างตัวเลข + คำเร่งด่วน(ก่อนหมด/เฉพาะวันนี้) + สั่งกดตะกร้าตรงๆ] — ${NO_PRICE_RULE} · ${NO_REPEAT_RULE} · ห้ามเกริ่นยาว ห้ามอ่านสรรพคุณเป็นลิสต์ ห้ามเคลมเกินจริง(ขาวใน X วัน/รักษา/100%) บทพูดต้องจบไม่เกิน ${fmtT(speakEnd)} วิ ห้ามค้างกลางคำตอนคลิปจบ>"`;
...
- สินค้า: ${name} | ราคา: ${price} บาท (ข้อมูลประกอบเท่านั้น — ห้ามพูดตัวเลขนี้ในคลิป) | ขายแล้ว: ${sold}
...
${userBlock}${recentBlock}${gen && gen.len > 1 ? `- ความยาว ${dur} วินาที = ${gen.len} คลิปต่อเนื่องกัน คลิปละ 10 วินาที: ใน timeline ให้ระบุรอยต่อว่า "ท้ายช่วงก่อนค้างท่าไหน ช่วงถัดไปเริ่มจากท่านั้นเป๊ะ" กันภาพกระโดด ฉาก/ตัวละคร/แสงเดิมทั้งคลิป` : ''}
...
ส่งออกเป็น JSON ตาม schema นี้เป๊ะ (ภาษาไทยในทุก value, ละเอียดทุก field):
{
  ...
  "timeline": [
    ...
    {"time": "${fmtT(midT)}-${fmtT(speakEnd)}s", "beat": "ปิดการขาย — ความคุ้ม+เร่งด่วน+สั่งกดตะกร้า (บทพูดจบที่นี่ · ห้ามพูดราคา)", "action": "<สั่งกดตะกร้า ชูสินค้านิ่งกลางเฟรม/ชี้ลง — บทพูดต้องจบภายในช่วงนี้>", "shot": "<...>", "camera": "<...>", "emotion": "<...>"},
  ...
  "negative_prompt": ["ซับไตเติล", "ตัวหนังสือซ้อนบนจอ", "คำบรรยาย", "แคปชั่น", "kinetic typography", "title card", "โลโก้แบรนด์อื่น", "การตัดสลับฉาก", "อวัยวะหรือสินค้าบิดเบี้ยวผิดรูป", "แขนหรือมือเกิน 2 ข้าง", "นิ้วเกินหรือขาด", "สินค้าลอยหลุดจากมือ", "สินค้าเปลี่ยนขนาด/รูปทรงกลางคลิป", "พูดประโยคเดิมซ้ำหรือวนลูป", "พูดราคาหรือตัวเลขเงิน", "จบแบบตัดกลางคัน", "สีเพี้ยนสลับช็อต"]
}
...
ส่งออกเป็น JSON เป๊ะตาม schema นี้ (ภาษาไทยทุก value · โฟกัสการเคลื่อนไหว+เสียง · ห้ามมี field บรรยายภาพนิ่ง/หน้าตา/ฉาก/แสง):
{
  ...
  "timeline": [
    ...
    {"time": "${fmtT(midT)}-${fmtT(speakEnd)}s", "beat": "CLOSE", "action": "<พูดปิดการขาย+ความคุ้ม+เร่งด่วน (ห้ามพูดราคา/ตัวเลขเงิน) แล้วยกมือชี้นิ้วลงล่างชวนกดตะกร้า>"},
    {"time": "${fmtT(speakEnd)}-${fmtT(dur)}s", "beat": "FREEZE", "action": "<ค้างท่าชี้ตะกร้า เงียบ ไม่มีคำพูด micro-motion เบา>"}
  ],
  "motion_intensity": "low — ขยับธรรมชาติ ช้า ๆ น้อย ๆ (ยิ่งแรง หน้า/มือ/สินค้ายิ่งเพี้ยน)",
  "physics": "${PHYSICS_RULE}",
  ...
  "negative_prompt": ["เปลี่ยนใบหน้า/ตัวละครจากเฟรมเริ่ม", "วาดหน้า/ฉาก/สินค้าใหม่ไม่ตรงเฟรม", "ตัวละครออกนอกเฟรม", "มือ/นิ้วเกินหรือผิดรูป", "แขนหรือมือเกิน 2 ข้าง", "สินค้าลอยหลุดจากมือ", "สินค้าทะลุผ่านมือ", "สินค้าเปลี่ยนขนาด/รูปทรง/สีกลางคลิป", "อวัยวะบิดเบี้ยวหรือมอร์ฟ", "พูดประโยคเดิมซ้ำหรือวนลูป", "พูดราคาหรือตัวเลขเงิน", "ปากไม่ตรงเสียง", "ซับไตเติล", "ตัวหนังสือซ้อนบนจอ", "คำบรรยาย", "แคปชั่น", "kinetic typography", "การตัดสลับฉาก", "จบค้างกลางคำ"]
}
...
ตอบ JSON valid อย่างเดียว ห้าม markdown fence ห้ามคำอธิบายนอก JSON · ห้ามมี field บรรยายรูปร่าง/หน้าตา/สีผิว/ทรงผม/ฉาก/แสง (เฟรมจัดการแล้ว) · บีตรวม ${fmtT(dur)} วิเป๊ะ · บทพูดจบไม่เกิน ${fmtT(speakEnd)} วิ ห้ามค้างกลางคำ · camera/timeline(action)/dialogue_th/pacing ต้องสะท้อน "เอกลักษณ์สไตล์" ด้านบนให้ชัด ห้ามออกมากลางๆ จนเหมือนสไตล์อื่น`;
  ...
  try {
    ...
    await rememberScript(obj && obj.dialogue_th);   // เก็บไว้กันคลิปถัดไปเขียนบทซ้ำ
    return JSON.stringify(reinforce(obj, hasCharImage, gen, i2v), null, 2);
  } catch { return text.trim(); }
}

// เก็บบทพูดที่เพิ่งใช้ (ล่าสุด 12 อัน) — ป้อนกลับเข้า prompt รอบหน้าเป็นรายการ "ห้ามซ้ำ"
async function rememberScript(line) {
  const t = String(line || '').trim();
  if (!t) return;
  try {
    const cur = (await chrome.storage.local.get('flow_recent_scripts')).flow_recent_scripts || [];
    const next = cur.filter((x) => x !== t).concat(t).slice(-12);
    await chrome.storage.local.set({ flow_recent_scripts: next });
  } catch {}
}
...
const FIDELITY_RULES = [
  ...
  'ร่างกายมี 2 แขน 2 มือเท่านั้น มือละ 5 นิ้ว ห้ามงอกแขน/มือ/นิ้วเพิ่ม ห้ามข้อต่อบิดผิดทาง แม้ตอนขยับหรือเปลี่ยนมุม',
  'สินค้าต้องอยู่ในมือตลอด มีจุดสัมผัสชัดเจน ห้ามลอยเอง ห้ามหลุดมือ ห้ามทะลุผ่านมือ ขนาด/รูปทรงคงเดิมทุกเฟรม',
  'พูดครั้งเดียวจบ ห้ามพูดประโยคเดิมซ้ำหรือวนลูป · ห้ามพูดราคาหรือตัวเลขเงินใด ๆ',
];
...
async function buildFlowPrompt(product, dry = false, i2v = false) {
  ...
  if (styleFb && gen && gen.len > 1) {
    ...
    styleFb = `สร้างวิดีโอต่อเนื่อง ${gen.len} คลิป คลิปละ 10 วินาที ฉากเดียวกัน ตัวละครเดิม แสงเดิมทุกคลิป `
      ...
      + `"ของดีแบบนี้ไม่ต้องคิดเยอะ กดตะกร้าด้านล่างเลย!" จบด้วยชูสินค้านิ่งกลางเฟรม ยิ้มมองกล้องค้างไว้ `
